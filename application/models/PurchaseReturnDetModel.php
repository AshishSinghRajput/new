...
class PurchaseReturnDetModel extends CI_Model
{
    //put your code here
    private $table_name = 'purchase_return_det';

    public function get_record($finyear_id, $store_id = '', $purchase_return_det_id = '', $purchase_return_mst_id = '')
    {
        $query = "SELECT `purchase_return_det_id`, `store_id`, `purchase_return_mst_id`, `product_id`, `product_code`, `product_heading`, `quantity`, `packing_id`, `packing_title`, `unit_id`, `unit_title`, `mfg_date`, `expiry_date`, `batch_no`, `mrp_price`, `rate`, `total_mrp`, `total_rate`, `cgst`, `sgst`, `igst`, `total_amount`, `is_cancel`, `cancel_date`, `cancel_reason`, `finyear_id`, `created_date`, `created_time`, `created_by`, `created_name`, `created_user_agent`, `created_ip`, `updated_date`, `updated_time`, `updated_by`, `updated_name`, `updated_user_agent`, `updated_ip` FROM `purchase_return_det` ";
        $query .= "WHERE (`finyear_id` = '" . $finyear_id . "') ";
        $query .= "AND (`store_id` = '" . $store_id . "') ";

        if (($purchase_return_det_id != '') && ($purchase_return_det_id != 'null')) {
            $query.= "AND (`purchase_return_det_id` = '" . $purchase_return_det_id . "') ";
        }

        if (($purchase_return_mst_id != '') && ($purchase_return_mst_id != 'null')) {
            $query.= "AND (`purchase_return_mst_id` = '" . $purchase_return_mst_id . "') ";
        }
        $query.= "ORDER BY `purchase_return_det_id`";
