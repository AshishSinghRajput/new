...
class ManageDebitNote extends CI_Controller
{
    var $CI;
    ...
    public function index()
    {
        ...
        $load_permission = $this->customlib->setUsersLogs($login_info, SUPERVISOR_MANAGE_DEBIT_NOTE, base_url($this->uri->uri_string()));
        ...
        $main_menu['active'] = 'ManageDebitNote';
        $this->session->set_userdata($main_menu);

        $topbar = "Manage Debit Note";
        ...
        $data['debit_note_info'] = $this->DebitNoteMstModel->get_record($finyear_info->finyear_id, $login_info->store_id);

        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/debit_note_list', $data);
        $this->load->view('layout/footer', $data);
    }

    public function view($debit_note_mst_id)
    {
        $data['controller'] = $this;
        ...
        $debit_note_mst_id = base64_decode($debit_note_mst_id);
        $data['debit_note_mst_id'] = $debit_note_mst_id;

        $data['debit_note_info'] = $this->DebitNoteMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, $debit_note_mst_id)['0'];

        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/debit_note_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add($supplier_id = '')
    {
        ...
        $this->debit_note_validation(false);
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Supervisor/debit_note_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            $account_info = $this->AccountMstModel->get_record(ACCOUNT_DEBIT_NOTES)['0'];
            ...
            $debit_note_no_info = $this->DebitNoteMstModel->get_count($finyear_info->finyear_id, $login_info->store_id)['0'];
            $debit_note_no = $debit_note_no_info->total;

            $this->db->trans_start();
            
            $debit_note_data['is_credit'] = IS_NOT_DEFINE_SALES;
            $debit_note_data['debit_note_no'] = $debit_note_no;
            $debit_note_data['date'] = $this->customlib->get_YYYYMMDD($this->input->post('date'));
            $debit_note_data['store_id'] = $login_info->store_id;
            $debit_note_data['supplier_id'] = $this->input->post('supplier_id');
            $debit_note_data['dues_amount'] = $dues_amount;
            $debit_note_data['pay_amount'] = $this->input->post('pay_amount');
            $debit_note_data['adjustment'] = $this->input->post('adjustment');
            $debit_note_data['grand_total'] = $total_amount;
            $debit_note_data['round_off'] = round($total_amount, 0);
            $debit_note_data['amount_word'] = $this->customlib->number_words(round($this->input->post('pay_amount'), 0));
            $debit_note_data['remarks'] = $this->input->post('remarks'); 
            $debit_note_data['payment_mode_id'] = $this->input->post('payment_mode_id');
            $debit_note_data['payment_mode'] = $payment_mode_info->payment_mode;
            $debit_note_data['bank_id'] = $this->input->post('bank_id');
            $debit_note_data['transaction_no'] = $this->input->post('transaction_no');
            $debit_note_data['transaction_date'] = $this->customlib->get_YYYYMMDD($this->input->post('transaction_date'));  
            $debit_note_data['branch'] = $this->input->post('branch');
            $debit_note_data['status_id'] = 'Pending';
            $debit_note_data['status_date'] = date('Y-m-d', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $debit_note_data['status_remarks'] = ''; //$this->input->post('status_remarks');
            $debit_note_data['is_cancel'] = '0';
            $debit_note_data['cancel_date'] = date('Y-m-d', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $debit_note_data['cancel_reason'] = ''; //$this->input->post('status_remarks');
            $debit_note_data['finyear_id'] = $finyear_info->finyear_id; 
            $debit_note_data['created_date'] = date('Y-m-d', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $debit_note_data['created_time'] = date('H:i:s', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $debit_note_data['created_by'] = $login_info->users_id;
            $debit_note_data['created_name'] = $login_info->name;
            $debit_note_data['created_user_agent'] = $this->customlib->load_agent();
            $debit_note_data['created_ip'] = $this->input->ip_address();

            $debit_note_mst_id = $this->DebitNoteMstModel->add($debit_note_data);

            $account_mst_data['debit_note'] = $supplier_account_info->debit_note+$this->input->post('pay_amount');
            ...
            $account_det_data['invoice_no'] = $debit_note_no;
            ...
            $account_det_data['master_id'] = $debit_note_mst_id;
            ...
            if (($debit_note_mst_id > 0) && ($supplier_account_det_id > 0)) {
                $this->session->set_flashdata('ses_success', $this->lang->line('insert_confirmation_message'));
                redirect('Supervisor/ManageDebitNote');
            } else {
                $this->session->set_flashdata('error_msg', $this->lang->line('insert_update_error_message'));
                redirect('Supervisor/ManageDebitNote/add');
            }
        }
    }

    public function cancel($debit_note_mst_id) {
        $data['controller'] = $this;
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {                
                $debit_note_info = $this->DebitNoteMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, $debit_note_mst_id)['0'];
                $data['debit_note_info'] = $debit_note_info;
                if ($debit_note_info->is_cancel == '1') {
                    $this->session->set_flashdata('error_msg', $this->lang->line('cancel_error_message'));
                    redirect('Supervisor/ManageDebitNote');
                } else {
                    $this->load->view('layout/header', $data);
                    $this->load->view('Supervisor/debit_note_cancel', $data);
                    $this->load->view('layout/footer', $data);
                }
            ...
            $account_info = $this->AccountMstModel->get_record(ACCOUNT_DEBIT_NOTES)['0'];
            $supplier_account_info = $this->SupplierAccountMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, $debit_note_info->supplier_id)['0'];
            ...
            $total_amount = $debit_note_info->pay_amount+$debit_note_info->adjustment;
            $net_amount = $dues_amount+$total_amount;
            
            $this->db->trans_start();
            
            $debit_note_data['is_cancel'] = '1';
            $debit_note_data['cancel_date'] = $this->customlib->get_YYYYMMDD($this->input->post('cancel_date'));
            $debit_note_data['cancel_reason'] = $this->input->post('cancel_reason');
            $debit_note_data['updated_date'] = date('Y-m-d', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $debit_note_data['updated_time'] = date('H:i:s', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $debit_note_data['updated_by'] = $login_info->users_id;
            $debit_note_data['updated_name'] = $login_info->name;
            $debit_note_data['updated_user_agent'] = $this->customlib->load_agent();
            $debit_note_data['updated_ip'] = $this->input->ip_address();
            
            $debit_note_where['finyear_id'] = $finyear_info->finyear_id; 
            $debit_note_where['store_id'] = $login_info->store_id;
            $debit_note_where['supplier_id'] = $debit_note_info->supplier_id;
            $debit_note_where['debit_note_mst_id'] = $debit_note_mst_id;
            
            $this->DebitNoteMstModel->modify($debit_note_data, $debit_note_where);

            $account_mst_data['debit_note'] = $supplier_account_info->debit_note-$debit_note_info->pay_amount;
            $account_mst_data['adjustment'] = $supplier_account_info->adjustment-$debit_note_info->adjustment;
            $account_mst_data['net_amount'] = $net_amount;
            $account_mst_data['updated_date'] = date('Y-m-d', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $account_mst_data['updated_time'] = date('H:i:s', mktime(gmdate('H') + 5, gmdate('i') + 30, gmdate('s'), gmdate('m'), gmdate('d'), gmdate('Y')));
            $account_mst_data['updated_by'] = $login_info->users_id;
            ...
            $account_mst_where['finyear_id'] = $finyear_info->finyear_id; 
            $account_mst_where['store_id'] = $login_info->store_id;
            $account_mst_where['supplier_id'] = $debit_note_info->supplier_id;
            $account_mst_where['supplier_account_mst_id'] = $supplier_account_info->supplier_account_mst_id;
            ...
            $account_det_where['supplier_id'] = $debit_note_info->supplier_id;
            $account_det_where['supplier_account_mst_id'] = $supplier_account_info->supplier_account_mst_id;
            $account_det_where['master_id'] = $debit_note_mst_id;
    ...
    public function prints($debit_note_mst_id) {
        $data['controller'] = $this;
        ...
        $this->load->view('Supervisor/debit_note_prints', $data);
    }

    public function debit_note_validation($required = true) {

        $this->form_validation->set_message('required', '%s required');

        $this->form_validation->set_rules('debit_note_no', 'DebitNote No.', 'trim|required|max_length[10]');
