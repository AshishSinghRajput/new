...
class CreditSales extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view('Supervisor/credit_sales_list', $data);
    ...
    public function view($sales_mst_id)
    {
        ...
        $data['sales_mst_info'] = $this->SalesMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, IS_CREDIT_SALES, $sales_mst_id)['0'];

        $data['sales_det_info'] = $this->SalesDetModel->get_record($finyear_info->finyear_id, $login_info->store_id, '', $sales_mst_id);

        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/credit_sales_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Supervisor/credit_sales_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            $credit_sales_details = $this->session->userdata('credit_sales_details');
            ...
            for ($counter = 0; $counter < count($credit_sales_details); $counter++) {
                $product_info = $this->ProductMstModel->get_autocomplete($finyear_info->finyear_id, $login_info->store_id, '', $credit_sales_details[$counter]['product_id'])['0'];
                
                $total_quantity = $total_quantity + $credit_sales_details[$counter]['quantity'];
                $total_mrp = $total_mrp + ($credit_sales_details[$counter]['quantity']*$credit_sales_details[$counter]['mrp']);
                
                $rate = $credit_sales_details[$counter]['quantity']*$credit_sales_details[$counter]['rate'];
                ...
                //$rate = round($credit_sales_details[$counter]['rate']-$gst_amount, 2);
                //$total_rate = $total_rate + ($credit_sales_details[$counter]['quantity']*$credit_sales_details[$counter]['rate']);
            }
            ...
            $sales_mst_data['is_credit'] = IS_CREDIT_SALES;
            ...
            for ($counter = 0; $counter < count($credit_sales_details); $counter++) {
                ...
                $product_full_info = $this->ProductMstModel->get_full_record($credit_sales_details[$counter]['product_id'])['0'];

                $total_mrp = $credit_sales_details[$counter]['quantity']*$credit_sales_details[$counter]['mrp'];
                $total_rate = $credit_sales_details[$counter]['quantity']*$credit_sales_details[$counter]['rate'];
                ...
                $sales_det_data['product_id'] = $credit_sales_details[$counter]['product_id'];
                $sales_det_data['product_code'] = $product_info->product_code;
                $sales_det_data['product_heading'] = $product_info->heading;
                $sales_det_data['quantity'] = $credit_sales_details[$counter]['quantity'];
                ...
                $sales_det_data['mfg_date'] = $this->customlib->get_YYYYMMDD($credit_sales_details[$counter]['mfg_date']);
                $sales_det_data['expiry_date'] = $this->customlib->get_YYYYMMDD($credit_sales_details[$counter]['expiry_date']);
                $sales_det_data['batch_no'] = $credit_sales_details[$counter]['batch_no'];
                $sales_det_data['mrp_price'] = $credit_sales_details[$counter]['mrp'];
                $sales_det_data['rate'] = $credit_sales_details[$counter]['rate'];
                $sales_det_data['total_mrp'] = $total_mrp; //$credit_sales_details[$counter]['total_mrp'];
                $sales_det_data['total_rate'] = $total_rate; //$credit_sales_details[$counter]['total_rate'];
                $sales_det_data['cgst'] = $cgst_amount; //$credit_sales_details[$counter]['total_rate'];
                $sales_det_data['sgst'] = $sgst_amount; //$credit_sales_details[$counter]['total_rate'];
                $sales_det_data['igst'] = $igst_amount; //$credit_sales_details[$counter]['total_rate'];
                ...
                $product_account_mst_info = $this->ProductAccountMstModel->get_record(
                    ...
                    $credit_sales_details[$counter]['product_id'])['0'];

                $product_account_mst_data['sales'] = $product_account_mst_info->sales+$credit_sales_details[$counter]['quantity'];
                $product_account_mst_data['net_amount'] = $product_account_mst_info->net_amount
                -$credit_sales_details[$counter]['quantity'];
                ...
                $product_account_mst_where['product_id'] = $credit_sales_details[$counter]['product_id'];
                ...
                $product_account_det_data['product_id'] = $credit_sales_details[$counter]['product_id'];
                ...
                $product_account_det_data['quantity'] = $credit_sales_details[$counter]['quantity'];
                ...
                $product_account_det_data['mfg_date'] = $this->customlib->get_YYYYMMDD($credit_sales_details[$counter]['mfg_date']);
                $product_account_det_data['expiry_date'] = $this->customlib->get_YYYYMMDD($credit_sales_details[$counter]['expiry_date']);
                $product_account_det_data['batch_no'] = $credit_sales_details[$counter]['batch_no'];
                $product_account_det_data['mrp_price'] = $credit_sales_details[$counter]['mrp'];
                $product_account_det_data['rate'] = $credit_sales_details[$counter]['rate'];
                $product_account_det_data['total_mrp'] = $total_mrp; //$credit_sales_details[$counter]['total_mrp'];
                $product_account_det_data['total_rate'] = $total_rate; //$credit_sales_details[$counter]['total_rate'];
                ...
                $product_account_det_data['total_amount'] = $total_amount; //$credit_sales_details[$counter]['total_rate'];
    ...
    public function cancel($sales_mst_id) {
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {        
                $sales_mst_info = $this->SalesMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, IS_CREDIT_SALES, $sales_mst_id)['0'];
                ...
                if ($sales_mst_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Supervisor/credit_sales_cancel', $data);
                    $this->load->view('layout/footer', $data);
    ...
    public function prints($sales_mst_id) {
        ...
        $this->load->view('Supervisor/credit_sales_prints', $data);
    }
    ...
    {
        $session['credit_sales_details'] = $this->session->userdata('credit_sales_details');

        $credit_sales_details = $this->session->userdata('credit_sales_details');
        $found = false;
        for ($counter = 0; $counter < count($credit_sales_details); $counter++) {
            /*if (($this->input->post('sales_product_id') == $credit_sales_details[$counter]['product_id'])
            && ($this->input->post('batch_no') == $credit_sales_details[$counter]['batch_no'])
            && ($this->input->post('mrp') == $credit_sales_details[$counter]['mrp'])
            && ($this->input->post('rate') == $credit_sales_details[$counter]['rate'])
            ) {*/
            if ($this->input->post('product_id') == $credit_sales_details[$counter]['product_id']) {
                $found = true;
                $ss = $counter;
                break;
            }
        ...
        if ($found) {
            $session['credit_sales_details'][$ss]['quantity'] += $this->input->post('quantity');

        } else {
            $session['credit_sales_details'][] = array(
    ...
    public function remove_item($item_id)
    {
        ...
        for ($counter = 0; $counter < count($credit_sales_details); $counter++) {
            if ($counter != $item_id) {
                $session['credit_sales_details'][] = array(
                    'product_code' => $credit_sales_details[$counter]['product_code'],
                    'product_name' => $credit_sales_details[$counter]['product_name'],
                    'product_id' => $credit_sales_details[$counter]['product_id'],
                    'mfg_date' => $credit_sales_details[$counter]['mfg_date'],
                    'expiry_date' => $credit_sales_details[$counter]['expiry_date'],
                    'batch_no' => $credit_sales_details[$counter]['batch_no'],
                    'quantity' => $credit_sales_details[$counter]['quantity'],
                    'packing_title' => $credit_sales_details[$counter]['packing_title'],
                    'mrp' => $credit_sales_details[$counter]['mrp'],
                    'rate' => $credit_sales_details[$counter]['rate'],
                    'total_mrp' => $credit_sales_details[$counter]['total_mrp'],
                    'total_rate' => $credit_sales_details[$counter]['total_rate']
                );
            }
        }
        $this->session->unset_userdata('credit_sales_details');
        ...
        redirect(base_url('Supervisor/CreditSales/add'));
    }
