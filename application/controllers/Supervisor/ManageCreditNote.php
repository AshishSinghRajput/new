...
class ManageCreditNote extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $load_permission = $this->customlib->setUsersLogs($login_info, SUPERVISOR_MANAGE_CREDIT_NOTE, base_url($this->uri->uri_string()));
        ...
        $this->load->view('Supervisor/credit_note_list', $data);
    ...
    public function view($credit_note_mst_id)
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/credit_note_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add($supplier_id = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Supervisor/credit_note_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            $credit_note_data['is_credit'] = IS_NOT_DEFINE_SALES;
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {                
                ...
                if ($credit_note_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Supervisor/credit_note_cancel', $data);
                    $this->load->view('layout/footer', $data);
                }
    ...
    public function prints($credit_note_mst_id) {
        ...
        $this->load->view('Supervisor/credit_note_prints', $data);
    }
    ...
    public function credit_note_validation($required = true) {
        ...
        $this->form_validation->set_rules('credit_note_no', 'CreditNote No.', 'trim|required|max_length[10]');
