...
class ManageReceipt extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('Supervisor/receipt_list', $data);
    ...
    public function view($receipt_mst_id)
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/receipt_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add($supplier_id = '')
    {
        ...
        $data['sales_mst_info'] = $this->SalesMstModel->get_receipt($finyear_info->finyear_id, $login_info->store_id, '', $current_supplier_id);
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Supervisor/receipt_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            $receipt_data['is_credit'] = IS_NOT_DEFINE_SALES;
            ...
            $receipt_data['sales_mst_id'] = implode(', ', $sales_mst_id);
            ...
            for($record = 0; $record < count($sales_mst_id); $record++) {
                $sales_mst_data['is_receipt'] = '1';
                $sales_mst_data['receipt_mst_id'] = $receipt_mst_id;
                ...
                $sales_mst_where['supplier_id'] = $this->input->post('supplier_id');
                $sales_mst_where['sales_mst_id'] = $sales_mst_id[$record];
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {                
                ...
                if ($receipt_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Supervisor/receipt_cancel', $data);
                    $this->load->view('layout/footer', $data);
                }
    ...
    public function prints($receipt_mst_id) {
        ...
        $this->load->view('Supervisor/receipt_prints', $data);
    }
    ...
    public function receipt_validation($required = true) {
        ...
        $this->form_validation->set_rules('receipt_no', 'Receipt No.', 'trim|required|max_length[10]');
