...
class ManagePayment extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('Supervisor/payment_list', $data);
    ...
    public function add($supplier_id = '')
    {
        ...
        $current_purchase_mst_id = ''; //$supplier_id;
        $data['current_purchase_mst_id'] = $current_purchase_mst_id;

        $data['supplier_list'] = $this->SupplierMstModel->get_select($login_info->store_id);
        
        $data['purchase_mst_info'] = $this->PurchaseMstModel->get_payment($finyear_info->finyear_id, $login_info->store_id, '', $current_supplier_id);
        ...
        if ($this->form_validation->run() == false) {
            ...
            $purchase_mst_id = $this->input->post('purchase_mst_id');
            ...
            $payment_data['is_credit'] = IS_NOT_DEFINE_SALES;
            ...
            $payment_data['purchase_mst_id'] = implode(', ', $purchase_mst_id);
            ...
            for($record = 0; $record < count($purchase_mst_id); $record++) {
                $purcahse_mst_data['is_payment'] = '1';
                $purcahse_mst_data['payment_mst_id'] = $payment_mst_id;
                ...
                $purcahse_mst_where['supplier_id'] = $this->input->post('supplier_id');
                $purcahse_mst_where['purchase_mst_id'] = $purchase_mst_id[$record];
    ...
    public function payment_validation($required = true) {
        ...
        $this->form_validation->set_rules('payment_no', 'Payment No.', 'trim|required|max_length[10]');
