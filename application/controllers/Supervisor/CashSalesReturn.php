...
class CashSalesReturn extends CI_Controller {
    ...
    public function index()
    {
        ...
        $main_menu['active'] = 'CashSalesReturn';
        $this->session->set_userdata($main_menu);

        $topbar = 'Credit Sales Return';
        ...
        $data['sales_return_mst_info'] = $this->SalesReturnMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, IS_CASH_SALES);

        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/cash_sales_return_list', $data);
    ...
    public function view($sales_return_mst_id)
    {
        ...
        $data['sales_return_mst_info'] = $this->SalesReturnMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, IS_CASH_SALES, $sales_return_mst_id)['0'];
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Supervisor/cash_sales_return_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add() {
        ...
        $data['sales_mst_info'] = $this->SalesMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, IS_CASH_SALES);
        
        $this->sales_return_validation(false);
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Supervisor/cash_sales_return_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            $sales_mst_id = $this->input->post('sales_mst_id');
            ...
            $cash_sales_return_details = $this->session->userdata('cash_sales_return_details');
            ...
            for ($counter = 0; $counter < count($cash_sales_return_details); $counter++) {
                $product_info = $this->ProductMstModel->get_autocomplete($finyear_info->finyear_id, $login_info->store_id, '', $cash_sales_return_details[$counter]['product_id'])['0'];
                
                $total_quantity = $total_quantity + $cash_sales_return_details[$counter]['quantity'];
                $total_mrp = $total_mrp + ($cash_sales_return_details[$counter]['quantity']*$cash_sales_return_details[$counter]['mrp']);
                
                $rate = $cash_sales_return_details[$counter]['quantity']*$cash_sales_return_details[$counter]['rate'];
                ...
                //$rate = round($cash_sales_return_details[$counter]['rate']-$gst_amount, 2);
                //$total_rate = $total_rate + ($cash_sales_return_details[$counter]['quantity']*$cash_sales_return_details[$counter]['rate']);
            }
            ...
            $account_det_data['is_credit'] = IS_CASH_SALES;
            ...
            for ($counter = 0; $counter < count($cash_sales_return_details); $counter++) {
                ...
                $product_full_info = $this->ProductMstModel->get_full_record($cash_sales_return_details[$counter]['product_id'])['0'];

                $total_mrp = $cash_sales_return_details[$counter]['quantity']*$cash_sales_return_details[$counter]['mrp'];
                $total_rate = $cash_sales_return_details[$counter]['quantity']*$cash_sales_return_details[$counter]['rate'];
                ...
                $sales_return_det_data['product_id'] = $cash_sales_return_details[$counter]['product_id'];
                $sales_return_det_data['product_code'] = $product_info->product_code;
                $sales_return_det_data['product_heading'] = $product_info->heading;
                $sales_return_det_data['quantity'] = $cash_sales_return_details[$counter]['quantity'];
                ...
                $sales_return_det_data['mfg_date'] = $this->customlib->get_YYYYMMDD($cash_sales_return_details[$counter]['mfg_date']);
                $sales_return_det_data['expiry_date'] = $this->customlib->get_YYYYMMDD($cash_sales_return_details[$counter]['expiry_date']);
                $sales_return_det_data['batch_no'] = $cash_sales_return_details[$counter]['batch_no'];
                $sales_return_det_data['mrp_price'] = $cash_sales_return_details[$counter]['mrp'];
                $sales_det_data['rate'] = $cash_sales_return_details[$counter]['rate'];
                $sales_det_data['total_mrp'] = $total_mrp; //$cash_sales_return_details[$counter]['total_mrp'];
                $sales_det_data['total_rate'] = $total_rate; //$cash_sales_return_details[$counter]['total_rate'];
                $sales_det_data['cgst'] = $cgst_amount; //$cash_sales_return_details[$counter]['total_rate'];
                $sales_det_data['sgst'] = $sgst_amount; //$cash_sales_return_details[$counter]['total_rate'];
                $sales_det_data['igst'] = $igst_amount; //$cash_sales_return_details[$counter]['total_rate'];
                $sales_det_data['total_amount'] = $total_amount;
                ...
                $product_account_mst_info = $this->ProductAccountMstModel->get_record(
                    ...
                    $cash_sales_return_details[$counter]['product_id'])['0'];

                $product_account_mst_data['sales_return'] = $product_account_mst_info->sales_return+$cash_sales_return_details[$counter]['quantity'];
                $product_account_mst_data['net_amount'] = $product_account_mst_info->net_amount
                +$cash_sales_return_details[$counter]['quantity'];
                ...
                $product_account_mst_where['product_id'] = $cash_sales_return_details[$counter]['product_id'];
                ...
                $product_account_det_data['supplier_id'] = '0'; //$this->input->post('supplier_id');
                ...
                $product_account_det_data['product_id'] = $cash_sales_return_details[$counter]['product_id'];
                ...
                $product_account_det_data['quantity'] = $cash_sales_return_details[$counter]['quantity'];
                ...
                $product_account_det_data['mfg_date'] = $this->customlib->get_YYYYMMDD($cash_sales_return_details[$counter]['mfg_date']);
                $product_account_det_data['expiry_date'] = $this->customlib->get_YYYYMMDD($cash_sales_return_details[$counter]['expiry_date']);
                $product_account_det_data['batch_no'] = $cash_sales_return_details[$counter]['batch_no'];
                $product_account_det_data['mrp_price'] = $cash_sales_return_details[$counter]['mrp'];
                $product_account_det_data['rate'] = $cash_sales_return_details[$counter]['rate'];
                $product_account_det_data['total_mrp'] = $total_mrp; //$cash_sales_return_details[$counter]['total_mrp'];
                $product_account_det_data['total_rate'] = $total_rate; //$cash_sales_return_details[$counter]['total_rate'];
                ...
                $product_account_det_data['total_cgst_amount'] = $cgst_amount; //$this->input->post('cgst_amount');
                ...
                $product_account_det_data['total_sgst_amount'] = $sgst_amount; //$this->input->post('sgst_amount');
                ...
                $product_account_det_data['total_igst_amount'] = $igst_amount; //$this->input->post('igst_amount');
                $product_account_det_data['total_amount'] = $total_amount; //$cash_sales_return_details[$counter]['total_rate'];
    ...
    public function cancel($sales_return_mst_id) {
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {        
                $sales_return_mst_info = $this->SalesReturnMstModel->get_record($finyear_info->finyear_id, $login_info->store_id, IS_CASH_SALES, $sales_return_mst_id)['0'];
                ...
                if ($sales_return_mst_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Supervisor/cash_sales_return_cancel', $data);
                    $this->load->view('layout/footer', $data);
    ...
    public function prints($sales_return_mst_id) {
        ...
        $this->load->view('Supervisor/cash_sales_return_prints', $data);
    }
    ...
    {
        $session['cash_sales_return_details'] = $this->session->userdata('cash_sales_return_details');

        $cash_sales_return_details = $this->session->userdata('cash_sales_return_details');
        $found = false;
        for ($counter = 0; $counter < count($cash_sales_return_details); $counter++) {
            /*if (($this->input->post('sales_return_product_id') == $cash_sales_return_details[$counter]['product_id'])
            && ($this->input->post('batch_no') == $cash_sales_return_details[$counter]['batch_no'])
            && ($this->input->post('mrp') == $cash_sales_return_details[$counter]['mrp'])
            && ($this->input->post('rate') == $cash_sales_return_details[$counter]['rate'])
            ) {*/
            if ($this->input->post('product_id') == $cash_sales_return_details[$counter]['product_id']) {
                $found = true;
                $ss = $counter;
                break;
            }
        ...
        if ($found) {
            $session['cash_sales_return_details'][$ss]['quantity'] += $this->input->post('quantity');

        } else {
            $session['cash_sales_return_details'][] = array(
    ...
    public function remove_item($item_id)
    {
        ...
        for ($counter = 0; $counter < count($cash_sales_return_details); $counter++) {
            if ($counter != $item_id) {
                $session['cash_sales_return_details'][] = array(
                    'product_code' => $cash_sales_return_details[$counter]['product_code'],
                    'product_name' => $cash_sales_return_details[$counter]['product_name'],
                    'product_id' => $cash_sales_return_details[$counter]['product_id'],
                    'mfg_date' => $cash_sales_return_details[$counter]['mfg_date'],
                    'expiry_date' => $cash_sales_return_details[$counter]['expiry_date'],
                    'batch_no' => $cash_sales_return_details[$counter]['batch_no'],
                    'quantity' => $cash_sales_return_details[$counter]['quantity'],
                    'packing_title' => $cash_sales_return_details[$counter]['packing_title'],
                    'mrp' => $cash_sales_return_details[$counter]['mrp'],
                    'rate' => $cash_sales_return_details[$counter]['rate'],
                    'total_mrp' => $cash_sales_return_details[$counter]['total_mrp'],
                    'total_rate' => $cash_sales_return_details[$counter]['total_rate']
                );
            }
        }
        $this->session->unset_userdata('cash_sales_return_details');
        ...
        redirect(base_url('Supervisor/CashSalesReturn/add'));
    }
