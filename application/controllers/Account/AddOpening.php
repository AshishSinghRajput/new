...
class AddOpening extends CI_Controller {
    ...
    public function index() {
        ...
        $load_permission = $this->customlib->setUsersLogs($login_info, ACCOUNT_ADD_OPENING, base_url($this->uri->uri_string()));
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {
                ...
                $this->load->view('Account/opening_balance_add', $data);
            ...
            if(!empty($account_mst_info)) {
                ...
                $product_account_mst_data['min_qty'] = $total_opening+$this->input->post('min_qty');
            ...
            $product_account_det_data['rate'] = $this->input->post('rate');
            $product_account_det_data['total_mrp'] = $this->input->post('total_mrp');
            $product_account_det_data['total_rate'] = $this->input->post('total_rate');
	...
	public function checked_received_validation($required=true) {
        ...
        $this->form_validation->set_rules('rate', 'Rate', 'trim|required|is_natural|min_length[1]|max_length[10]');
        $this->form_validation->set_rules('total_rate', 'Total Rate', 'trim|required|is_natural|min_length[1]|max_length[10]');
