...
class ReportsPayment extends CI_Controller
{
    ...
    public function index() {
        ...
        $load_permission = $this->customlib->setUsersLogs($login_info, ACCOUNT_MANAGE_PAYMENT, base_url($this->uri->uri_string()));
        ...
        $this->load->view('Account/reports_payment', $data);
    ...
    public function view($payment_mst_id)
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Account/payment_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add($supplier_id = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Account/payment_add', $data);
                $this->load->view('layout/footer', $data);
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {                
                ...
                if ($payment_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Account/payment_cancel', $data);
                    $this->load->view('layout/footer', $data);
                }
    ...
    public function prints($payment_mst_id) {
        ...
        $this->load->view('Account/payment_prints', $data);
    }
