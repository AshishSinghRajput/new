...
class ManagePO extends CI_Controller {
    ...
    public function index()
    {
        ...
        $load_permission = $this->customlib->setUsersLogs($login_info, ACCOUNT_MANAGE_PURCHASE_ORDER, base_url($this->uri->uri_string()));
    ...
    public function view($po_mst_id)
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Account/po_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Account/po_add', $data);
                $this->load->view('layout/footer', $data);
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {        
                ...
                if ($po_mst_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Account/po_cancel', $data);
                    $this->load->view('layout/footer', $data);
    ...
    public function prints($po_mst_id) {
        ...
        $this->load->view('Account/po_prints', $data);
    }
    ...
    public function remove_item($item_id)
    {
        ...
        redirect(base_url('Account/ManagePO/add'));
    }
