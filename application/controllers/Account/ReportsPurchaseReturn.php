...
class ReportsPurchaseReturn extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('Account/reports_purchase_return', $data);
    ...
    public function view($purchase_return_mst_id)
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Account/purchase_return_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Account/purchase_return_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            for ($counter = 0; $counter < count($purchase_return_details); $counter++) {
                ...
                $purchase_return_det_data['total_mrp'] = $purchase_return_details[$counter]['total_mrp'];
                $purchase_return_det_data['total_rate'] = $purchase_return_details[$counter]['total_rate'];
                ...
                $product_account_det_data['total_mrp'] = $purchase_return_details[$counter]['total_mrp'];
                $product_account_det_data['total_rate'] = $purchase_return_details[$counter]['total_rate'];
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {        
                ...
                if ($purchase_return_mst_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Account/purchase_return_cancel', $data);
                    $this->load->view('layout/footer', $data);
    ...
    public function prints($purchase_return_mst_id) {
        ...
        $this->load->view('Account/purchase_return_prints', $data);
    }
    ...
    public function ajaxRequestPost()
    {
        ...
        $found = false;
        for ($counter = 0; $counter < count($purchase_return_details); $counter++) {
            if (($this->input->post('purchase_return_product_id') == $purchase_return_details[$counter]['product_id'])
             && ($this->input->post('batch_no') == $purchase_return_details[$counter]['batch_no'])
             && ($this->input->post('mrp') == $purchase_return_details[$counter]['mrp'])
             && ($this->input->post('rate') == $purchase_return_details[$counter]['rate'])
             ) {
                $found = true;
                $ss = $counter;
                break;
        ...
        if ($found) {
            $session['purchase_return_details'][$ss]['quantity'] += $this->input->post('purchase_return_order_qty');
    ...
    public function remove_item($item_id)
    {
        ...
        redirect(base_url('Account/ManagePurchaseReturn/add'));
    }
