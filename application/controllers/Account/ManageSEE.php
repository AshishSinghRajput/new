...
class ManageSEE extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->load->view('Account/see_list', $data);
    ...
    public function view($see_mst_id)
    {
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('Account/see_view', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == false) {
            if (!$this->input->post('submit')) {
                $this->load->view('layout/header', $data);
                $this->load->view('Account/see_add', $data);
                $this->load->view('layout/footer', $data);
            ...
            for ($counter = 0; $counter < count($see_details); $counter++) {
                ...
                $see_det_data['total_mrp'] = $see_details[$counter]['total_mrp'];
                $see_det_data['total_rate'] = $see_details[$counter]['total_rate'];
                ...
                $product_account_det_data['total_mrp'] = $see_details[$counter]['total_mrp'];
                $product_account_det_data['total_rate'] = $see_details[$counter]['total_rate'];
		...
		if($this->form_validation->run()==false) {
            if(!$this->input->post('submit')) {        
                ...
                if ($see_mst_info->is_cancel == '1') {
                    ...
                    $this->load->view('layout/header', $data);
                    $this->load->view('Account/see_cancel', $data);
                    $this->load->view('layout/footer', $data);
    ...
    public function prints($see_mst_id) {
        ...
        $this->load->view('Account/see_prints', $data);
    }
    ...
    public function remove_item($item_id)
    {
        ...
        redirect(base_url('Account/ManageSEE/add'));
    }
